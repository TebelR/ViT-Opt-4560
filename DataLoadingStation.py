...
class DataLoadingStation:
    ...
    def load_data_detection(self, path):
        self.dataset_train_detection = DetectionDataset(
            os.path.join(path, "detection", "images", "train"),
            os.path.join(path, "detection", "labels", "train")
        )

        self.dataset_validate_detection = DetectionDataset(
            os.path.join(path, "detection", "images", "valid"),
            os.path.join(path, "detection", "labels", "valid")
        )

        self.dataset_test_detection = DetectionDataset(
            os.path.join(path, "detection", "images", "test"),
            os.path.join(path, "detection", "labels", "test")
        )